...
// POST /api/sync-store/run  { storeId: string }
// Triggers an immediate sync for a configured store
export async function POST(req: NextRequest) {
  ...
  const { connectDB, SyncStoreModel, ProductModel } = await import("@/lib/mongodb");
  await connectDB();

  const store = await SyncStoreModel.findById(storeId);
  ...
  while (true) {
    ...
    try {
      ...
      return NextResponse.json({ error: `Could not reach ${base}` }, { status: 400 });
    }
    if (!res.ok) break;
  ...
  const ops = all.map((p, i) => {
    ...
    const product = {
      id: String(p.id ?? `sync-${store._id}-${i}`),
  ...
  await SyncStoreModel.findByIdAndUpdate(storeId, {
    $set: { lastSync: new Date(), lastCount: all.length },
  });
